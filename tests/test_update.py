...
def test_update_called_with_the_right_values(mock_broker, client, demo_service):
    mock_broker.catalog.return_value = demo_service
    mock_broker.update.return_value = UpdateServiceSpec(operation="operation")
    ...
    actual_service_id, actual_details, actual_async = mock_broker.update.call_args[0]
    assert actual_service_id == "here-service-instance-id"
    assert actual_async == True

    assert type(actual_details) == UpdateDetails
    assert type(actual_details.previous_values) == PreviousValues
    ...
    assert actual_details.previous_values.plan_id == "old-plan-guid-here"
    assert actual_details.previous_values.service_id == "service-guid-here"
    assert actual_details.previous_values.organization_id == "org-guid-here"
    assert actual_details.previous_values.space_id == "space-guid-here"
